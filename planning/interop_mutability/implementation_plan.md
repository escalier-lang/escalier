...
| 4   | Strong signals (tier 3)                | 🚧     | 3           | `classifyExplicitSignal` handles getters/setters, `readonly` props, `Readonly<T>` wrappers, `this: Readonly<T>`, and the well-known-symbol allow-list. Pending: extend `dts_parser` to surface unresolved type-reference names / `this` parameter modelling where missing.                                                                                               |
