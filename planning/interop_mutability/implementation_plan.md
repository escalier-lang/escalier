...
| §6.B   | **Always-immutable stdlib.** Per-class `.esc` files for `Number`, `BigInt`, `String`, `Boolean`, `Symbol`, `Promise` under `data/builtins/`. Every instance method declares `self`. Drop the matching `String` entry from `nonMutatingOverrides` only once the prelude carries the override's answer: a receiver `Classify` marks non-mutating does not reach the `.d.ts`-loaded lib types, so until `UpdateMethodMutability` reads the store or is deleted, the entry is what makes `String.charAt` callable on a non-mut receiver. Fixture per class asserting non-mut receiver. | 6 classes, ~150–250 method signatures, 6 fixtures                                                     | ⬜      |
| §6.C   | **Mixed-mutability stdlib, grouped by ES revision.** One sub-PR per `data/builtins/{es5,es2015,es2017,es2021,dom}.esc` covering the per-revision class list in §6.2. Each method annotated per ECMAScript spec / MDN. Drop the matching `nonMutatingOverrides` entries (`Object`, `Function`, `Console`, `Body`, `Request`, `Response`) as each class's override lands, under the §6.B condition on the prelude. | ~17 classes (incl. 9 typed arrays as a template), ~500–800 method signatures, ~15 fixtures, 5 sub-PRs | ⬜      |
...
| §6.F   | **Drop remaining bootstrap entries.** After §6.B/C cover the same classes, delete the remaining entries from the `nonMutatingOverrides` Go map and the `applyMethodMutability` second pass on non-`*Constructor` classes that has become dead code. | ~50–100 LOC net deletion                                                                              | ⬜      |
...
- **Built-ins** — classes that don't ship a `Readonly*` variant in
  ...
  **Bootstrap state (post-#612, post-#614).** A subset of these —
  ...
  the legacy `nonMutatingOverrides` Go map (see
