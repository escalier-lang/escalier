...
- **G3 — Bare reference annotation reborrows its initializer** (~150).
  - **Files:** `solver/infer_type_ann.go`, `solver/constrain.go`, `solver/infer_decl.go`,
    plus the D4 display path and `solver/infer_ann_test.go`.
  - **Background:** the new solver diverges from `internal/checker` on a local immutable
    view of a borrow. A bare `{x: number}` annotation lowers to an owned object type with
    `Lt` nil. Binding a `mut` borrow into it — `val q: {x: number} = p` for
    `p: mut {x: number}` — constrains `mut 'l {x} <: {x}`, a borrow into an owned slot,
    which trips `BorrowEscapeError`. The old checker accepts this. It has no borrow-escape
    concept: it unifies by unwrapping the `MutType` and lets only the liveness-based
    transition check govern the binding, which passes when `p` is dead afterward. So the
    new solver rejects a binding that is sound — a local immutable view of `p` that dies
    within `p`'s lifetime never escapes. The annotation should constrain shape only;
    whether `q` is owned or a borrow is inferred from its initializer and use.
  - **Algorithm:**
    1. Lower a bare object/tuple annotation in reference position as an immutable borrow
       with a fresh lifetime variable rather than an owned slot.
    2. When the initializer is itself a reference, add the reborrow constraint: the
       target's fresh lifetime is constrained `<:` the source's lifetime, through the same
       `constrainLt` step the borrow-into-borrow arm already runs
       (`TestInferBorrowIntoBorrowArg`).
    3. Let the lifetime sort decide escape. A binding that escapes, returned or stored into
       module state, carries D3's escape constraint forcing its lifetime `<: 'static`. That
       conflicts with target `<:` source when the source is not static, so the escape error
       fires correctly. A binding that stays local and dies within the source's lifetime has
       no conflict and is accepted, matching the old checker.
  - **Depends on G2.** The escape verdict in step 3 relies on D3's escape constraints being
    complete, the global-write gaps G2 finishes. It resolves a value's borrows through the
    same `VarID` ↔ `soltype`-borrow bridge G2 builds. Landing it before G2 would either
    over-accept an escaping reborrow, which is unsound, or duplicate the escape logic G2 is
    about to centralize.
  - **Blast radius.** This turns the D2 decision "a bare object annotation is owned" into
    "owned or borrow, inferred from the initializer". Update D4 display and elision so a
    reborrowed binding renders its inferred lifetime, or elides it when the lifetime reaches
    no output. Re-check the construction-site excess-member tests in `infer_ann_test.go`,
    which assume a bare annotation is owned.
  - **Accept:** `val q: {x: number} = p` for `p: mut {x: number}` type-checks with no error
    when `q` is local, and still errors when `q` escapes through a return or a module-level
    store. The mut→immutable transition check continues to fire on liveness as before. Once
    this lands, replace the `source_dead_no_transition` arm of
    `TestTransitionWiringReportsRule1Error`, which today asserts the divergent escape error.
