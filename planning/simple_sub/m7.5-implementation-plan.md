...
`std:array` to `<stdlib-dir>/std/array.esc`, and the `@js` loader rules.
...
- **`.esc` is the ingestion channel, not `.d.ts`.** The milestone describes
  ingestion as `dts_parser` → `dts_to_esc.ConvertModule` → `*ast.Module`. That is the
...
- **Named imports from a pseudo-package are rejected today.** The milestone's
  ...
  `import "std:array"` plus member access through the bound namespace. This plan
  follows the shipped design, and PR2 records the wording fix to the milestone.
  Adding named imports is a separable change to both checkers and is not
  scheduled here. FR5 names it as the answer if the qualified form proves too
  long.
...
13. **The npm `.d.ts` channel.** `dts_parser` → `dts_to_esc.ConvertModule` →
...
- **`bindStdlibLocal`** binds the package under the lowercased last URI segment
  and stops there. Every export is a member of that one binding, so a class is
  reached as `date.Date`.
  - **FR5 retires the single-class shortcut here.** An earlier revision lifted a
    class out of a package whose name matched it. Builtins §7
    ([#1232](https://github.com/escalier-lang/escalier/issues/1232) item 4) owed
    this PR the merge that would have put the package's other exports onto such a
    binding. Dropping the shortcut removes the need for the merge, so item 4
    closes with nothing to implement. FR5 records the three reasons.
...
**Accept.** `import "std:shapes"` against the test tree binds `shapes` and
`val s: shapes.Shapes` resolves to the ingested class, while a bare `Shapes` is
unbound; `import "std:bogus"` reports an unknown-package diagnostic naming the
searched directory; `import "bogus:array"` reports an unknown-scheme diagnostic
listing the three recognized schemes; a file that names an imported package's
class without qualifying it is an unbound-name error, proving there is no ambient
surface and no lifted class.
...
- **Reuse the AST-producing front half.** `dts_parser` parse →
  ...
  ([module.go:176](../../internal/dts_to_esc/module.go)), then hand that module to
