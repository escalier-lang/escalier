...
- Remove the `nonMutatingOverrides` entries that §6 proved redundant for
  ...
  **Deferred to the M12 flip.** The second condition is the one to wait on.
  ...
  the solver reads those files rather than the spec. Its own prelude seeds
  operator bindings and opaque stdlib placeholders and reads no `.d.ts`, so
  there is no ambient global surface for a receiver fact to apply to. The
  entries go out with their one reader.
...
`internal/dts_to_esc`, which is the converter `generate` runs:
...
1. `Classify`, the cascade over a parsed `.d.ts` class member. The facts sit
   ...
   authoritative.
2. `ReceiverMutates`, which answers from an owner and a member key. Trio
   fusion reads it, since a class fused from interface signatures has no
   `ClassMember` to feed the cascade.

Nothing in `internal/checker` or `internal/solver` reads a fact. The spec
analysis is an input to generation rather than to type-checking.
...
`generate` derives the facts from the graph `internal/ecma262` embeds, so a
run with no `--cfg` classifies from them and the committed tree under
`internal/interop/data` carries their answers. `--cfg` names a different graph
to classify from and reports on it, which is how a spec bump is previewed.
