...
| 6   | Converter productionization                          | FR10        | 🚧      | §5         | PR A landed: hand-maintained partition map ([internal/interop/partition.go](../../internal/interop/partition.go)) with `Route` + DOM residual + unmapped-symbol fail-safe; partition pipeline ([internal/interop/partition_writer.go](../../internal/interop/partition_writer.go)) that buckets, interface-/namespace-merges across input files, converts each bucket, and writes the partitioned tree under `<out>/std/`, `<out>/web/`, `<out>/node/`; `dts_to_esc partition <lib-dir> <out-dir>` subcommand. lib.es5.d.ts smoke gate runs end-to-end (8/17 packages parse roundtrip; the rest surface printer/parser gaps that §7 hand-edits and PR B's `--check` mode will close). PR B landed check 1 (missing declarations and members) plus additive write mode ([internal/interop/rerun.go](../../internal/interop/rerun.go), `dts_to_esc check` / `dts_to_esc regenerate`); its §6.4 checks 2 and 3 run the solver's `constrain` over `soltype` and wait on SimpleSub M7.5. PR C (TS-version-bump workflow) and PR D (split the converter out of the `type_system`-importing half of `internal/interop`) still outstanding. |
...
B. **`--check` mode + re-run semantics** (6.4) 🚧. Adds additive
   ...
   the shared diff in
   [internal/interop/rerun.go](../../internal/interop/rerun.go)
