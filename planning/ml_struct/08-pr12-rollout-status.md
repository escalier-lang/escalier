...
| PR8 | A union of refs differing only in lifetime factors to one ref. `(mut 'a T) \| (mut 'b T)` becomes `mut ('a \| 'b) T`. Negating a ref wrapper is forbidden by construction, so the `¬Ref` polarity obligation never arises. | [#1120](https://github.com/escalier-lang/escalier/pull/1120) |
