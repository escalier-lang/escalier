...
import { Emitter, IEvent, Uri } from 'monaco-editor-core';
...
export const escalierDefaults: LanguageServiceDefaults = new LanguageServiceDefaultsImpl(
	'esaclier',
	diagnosticDefault,
	modeConfigurationDefault
);

export const getWorker = (): Promise<(...uris: Uri[]) => Promise<EscalierWorker>> => {
	import('./escalier-mode').then((mode) => mode.getWorker());
}

// export to the global based API
(<any>languages).escalier = { escalierDefaults, getWorker };
...
languages.register({
	id: 'escalier',
	extensions: ['.esc'],
	aliases: ['Escalier', 'escalier'],
	mimetypes: ['application/escalier']
});

languages.onLanguage('escalier', () => {
	import('./escalier-mode').then((mode) => {
        mode.setupMode(escalierDefaults)
    });
});
