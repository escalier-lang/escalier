...
test.describe('App Loading', () => {
  test('default load shows hello-world example', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /^▸ bin$|^▾ bin$/ })).toBeVisible();
    await expect(page.getByRole('button', { name: /^▸ lib$|^▾ lib$/ })).toBeVisible();
  ...
  test('invalid example falls back to hello-world with warning', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /build/ })).toBeVisible({ timeout: 45_000 });
