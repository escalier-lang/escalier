...
test.describe('File Explorer', () => {
    ...
    test('create new file via header button', async ({ page }) => {
        ...
        await expect(
            page.getByRole('button', { name: 'test.esc' }),
        ).toBeVisible();
    ...
    test('create new folder via header button', async ({ page }) => {
        ...
        await expect(
            page.getByRole('button', { name: /^▸ src$|^▾ src$/ }),
        ).toBeVisible();
    });
    ...
    test('rename file via context menu', async ({ page }) => {
        ...
        await page
            .getByRole('button', { name: 'main.esc' })
            .click({ button: 'right' });
        ...
        await expect(
            page.getByRole('button', { name: 'main.esc' }),
        ).toBeHidden();
        await expect(
            page.getByRole('button', { name: 'entry.esc' }),
        ).toBeVisible();
    });
