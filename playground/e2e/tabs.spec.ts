...
test.describe('Tab Management', () => {
  ...
  test('close tab via close button', async ({ page }) => {
    ...
    // Tab should be gone
    await expect(inputTablist.getByRole('tab', { name: /index\.esc/ })).toBeHidden();
  });
  ...
  test('move tab to right side via context menu', async ({ page }) => {
    ...
    await inputTablist.getByRole('tab', { name: /index\.esc/ }).click({ button: 'right' });
    ...
    await expect(inputTablist.getByRole('tab', { name: /index\.esc/ })).toBeHidden();
    await expect(outputTablist.getByRole('tab', { name: /index\.esc/ })).toBeVisible();
  });
});
