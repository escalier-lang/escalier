...
func convertStatement(stmt dts_parser.Statement, ctx convCtx) (ast.Decl, error) {
	switch s := stmt.(type) {
...
func convertClassDecl(dc *dts_parser.ClassDecl, ctx convCtx) (*ast.ClassDecl, error) {
	// Convert type parameters
	...
	for _, member := range dc.Members {
		...
		case *dts_parser.ConstructorDecl:
			...
			ctorResult := Classify(ClassifyContext{Member: m, ClassName: dc.Name.Name, ModulePath: ctx.modulePath, Registry: ctx.registry})
		...
		case *dts_parser.MethodDecl:
			elem, err := convertMethodDecl(m, dc.Name.Name, ctx)
			if err != nil {
		...
		case *dts_parser.GetterDecl:
			elem, err := convertGetterDecl(m, dc.Name.Name, ctx)
			if err != nil {
		...
		case *dts_parser.SetterDecl:
			elem, err := convertSetterDecl(m, dc.Name.Name, ctx)
			if err != nil {
