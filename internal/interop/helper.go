...
func convertMethodDecl(md *dts_parser.MethodDecl, className string, ctx convCtx) (*ast.MethodElem, error) {
	...
	if !md.Modifiers.Static {
		result := Classify(ClassifyContext{Member: md, ClassName: className, ModulePath: ctx.modulePath, Registry: ctx.registry})
...
// className is the enclosing class name, passed to Classify for tier-2 signals.
func convertGetterDecl(gd *dts_parser.GetterDecl, className string, ctx convCtx) (*ast.GetterElem, error) {
	// Convert property key to object key
	...
	if !gd.Modifiers.Static {
		result := Classify(ClassifyContext{Member: gd, ClassName: className, ModulePath: ctx.modulePath, Registry: ctx.registry})
...
// className is the enclosing class name, passed to Classify for tier-2 signals.
func convertSetterDecl(sd *dts_parser.SetterDecl, className string, ctx convCtx) (*ast.SetterElem, error) {
	// Convert property key to object key
	...
	if !sd.Modifiers.Static {
		result := Classify(ClassifyContext{Member: sd, ClassName: className, ModulePath: ctx.modulePath, Registry: ctx.registry})
