...
follows. `export declare type <name>` does not parse without `= …`, so a
dropped type alias is named with the `val` form too; only the name is
read, so the keyword makes no claim about the upstream kind.
