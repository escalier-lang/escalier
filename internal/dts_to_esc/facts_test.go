...
//
// The comparison runs over the pinned lib set, which is the input `generate`
...
func TestStdReceiversMatchTheFacts(t *testing.T) {
	...
	// A full run rather than a bare partition: the overlay carries the drop
	// list the unmapped-symbol fail-safe reads, so a partition without it
	// aborts on the first declaration no package claims.
	...
	snaps.MatchInlineSnapshot(t, compared, snaps.Inline("int(232)"))
}
