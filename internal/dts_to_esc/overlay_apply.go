...
// ast.ObjTypeAnnElem declare the pair, and a variant with no doc of its
// own answers "" and ignores the set.
...
// the member-level counterpart of carryDeclMetadata.
//
// An overlay member carrying its own doc comment keeps it. The upstream
// prose fills in only where the overlay wrote none, which is why a doc
// edit upstream reaches the output whether or not an overlay replaces
// the member it sits above.
