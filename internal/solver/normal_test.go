...
func TestDNFRoundTrip(t *testing.T) {
	tests := []struct {
		...
		{
			name: "two records differing in one field's marker keep both atoms",
			in:   "{x: number} | {x?: number}",
			want: "{x: number} | {x?: number}",
		},
		{
...
// one through Conjunct.neg and one through the merge rules — so agreeing is a
// statement about the module, not about the printer.
func TestDeMorgan(t *testing.T) {
	tests := []struct {
		...
		{
			name: "two records that neither absorb nor fuse",
			operands: func(t *testing.T, c *Context) (soltype.Type, soltype.Type) {
				return parseType(t, "{x: number}"), parseType(t, "{y: number}")
			},
		},
		{
			name: "two disjoint primitives, where both sides collapse to a lattice bound",
			operands: func(t *testing.T, c *Context) (soltype.Type, soltype.Type) {
...
// exact fusions, and taking either one rules the other out, so a merge that fused
// the first pair it happened to reach would answer differently per order. The
// three arrows fuse either their two number-domain arms, meeting the codomains, or
// their two boolean-codomain arms, joining the domains. Scanning in canonical
// order is what settles which fusion a given member SET reaches.
...
// meet to. One tag in a conjunct is the base; two unrelated tags are kept apart by
// the stub glbClass, so the conjunct has no single base until that PR lands.
func TestBaseReadsTheSingleClassTag(t *testing.T) {
	c := &Context{}
	point := &soltype.ClassType{Name: "Point", TypeArgs: nil, LifetimeArgs: nil, Lt: nil, Final: false, Variant: false}
	line := &soltype.ClassType{Name: "Line", TypeArgs: nil, LifetimeArgs: nil, Lt: nil, Final: false, Variant: false}
	...
	// Two tags naming one class fuse, so the conjunct still has a single base.
	same := c.mkDNF(newIntersection(nil, []soltype.Type{point, &soltype.ClassType{
		Name: "Point", TypeArgs: nil, LifetimeArgs: nil, Lt: nil, Final: false, Variant: false,
	}}), soltype.Positive)
...
// but one, and would fuse two conjuncts that negate different tags.
func TestNominalAtomsStayDistinct(t *testing.T) {
	tag := func(name string) *soltype.ClassType {
		return &soltype.ClassType{
			Name: name, TypeArgs: nil, LifetimeArgs: nil, Lt: nil, Final: false, Variant: false,
		}
	}
	point, line, arc := tag("Point"), tag("Line"), tag("Arc")
	...
	t.Run("two conjuncts negating different tags are not fused", func(t *testing.T) {
		c := &Context{}
...
// inner is not borrowable, which is right for coalescing and would silently
// drop mutability here.
