...
func TestInferCallExactTooManyArgsSingleDiagnostic(t *testing.T) {
	...
	require.Equal(t, "3:11-3:21: Too many arguments: expected at most 2, but got 3", msgWithSpan(t, errs[0]))
}
...
func TestInferOptionalParamCallArity(t *testing.T) {
	...
	t.Run("overflow trips the lint", func(t *testing.T) {
		...
		`)
		require.Len(t, errs, 1)
		require.Equal(t, "3:12-3:22: Too many arguments: expected at most 2, but got 3", msgWithSpan(t, errs[0]))
	})
}
...
func TestInferNonTrailingOptionalRequiresAllArgs(t *testing.T) {
	...
	`)
	require.Len(t, errs, 1)
	require.Equal(t, "3:11-3:15: Not enough arguments: expected at least 2, but got 1", msgWithSpan(t, errs[0]))
}
...
func TestInferInexactFunctionValue(t *testing.T) {
	...
	t.Run("direct call still rejects extra args", func(t *testing.T) {
		...
		`)
		require.Len(t, errs, 1)
		require.Equal(t, "3:12-3:19: Too many arguments: expected at most 1, but got 2", msgWithSpan(t, errs[0]))
	})
}
...
func TestInferTooFewArgsRespectsOptional(t *testing.T) {
	...
	`)
	require.Len(t, errs, 1)
	require.Equal(t, "3:11-3:14: Not enough arguments: expected at least 1, but got 0", msgWithSpan(t, errs[0]))
}
