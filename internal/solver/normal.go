...
// in. That matters because a set of atoms can admit several distinct exact
// fusions and the first one taken rules out the others. `(number -> string) &
// (number -> boolean) & (string -> boolean)` fuses either its first two arms, by
// their shared domain, or its last two, by their shared codomain, and the two
// results are equal types written differently. Scanning in canonical order settles
// which one a given SET of atoms reaches, so permuting the source members cannot
...
// replacePair returns a copy of atoms with positions i and j dropped and the
// fused atom appended. i must be less than j.
func replacePair(atoms []soltype.Type, i, j int, fused soltype.Type) []soltype.Type {
	...
	out = append(out, atoms[i+1:j]...)
	out = append(out, atoms[j+1:]...)
...
// narrower than its primitive, so the literal wins, and two distinct literals are
// disjoint. Two equal atoms never reach here, since meetAtoms answers those first.
func meetValueAtoms(a, b soltype.Type) (soltype.Type, bool) {
	...
	// Two literals of one family that are not equal, such as `5` and `6`.
	return &soltype.NeverType{}, true
}
...
// meetObjects fuses two object atoms field by field. The fusion is exact for two
// INEXACT objects: each denotes the values carrying its fields plus anything else,
// so their meet is the values carrying both field sets plus anything else, which
// is the merged inexact object. It is exact for two EXACT objects over the same
// field names, whose meet narrows each shared field.
//
// Two exact objects over DIFFERENT field names keep both atoms. Their meet is
// really `never`, since an exact object carries no field its type does not name,
// but deciding that is PR7's exactness-aware merge; keeping both atoms is sound
// meanwhile. TODO(#1064).
//
// Two objects whose Inexact flags disagree keep both atoms for the same reason.
func (c *Context) meetObjects(a, b *soltype.ObjectType) (soltype.Type, bool) {
	...
	for _, p := range pa {
		...
		// A required field and an optional one of the same name denote different value
		// sets, and so do a readonly field and a writable one. Fusing them would have to
		// pick a marker, so the two atoms stay separate instead.
		if p.Optional != q.Optional || p.Readonly != q.Readonly {
			return nil, false
		}
...
// joinObjects fuses two object atoms under a union. The fusion is exact only when
// the two carry the same field names and differ in AT MOST ONE field's type:
...
//
// Two differing fields break that. `{x: A, y: C} | {x: B, y: D}` is not
// `{x: A | B, y: C | D}`, which would also admit a record pairing an x from A with
// a y from D, a record neither member admits. Differing field names break it too,
// which is why `{x: number} | {y: number}` keeps both atoms.
...
// meetTuples fuses two tuple atoms element by element. Two tuples of the same
// length and exactness denote value sets that differ only per position, so
// meeting each position is exact. A length or exactness difference keeps both
// atoms.
func (c *Context) meetTuples(a, b *soltype.TupleType) (soltype.Type, bool) {
	if !fusableTuples(a, b) {
		return nil, false
	}
...
// joinTuples fuses two tuple atoms under a union. As with joinObjects the fusion
// is exact only when the two differ in at most one position, since widening two
// positions at once would admit a tuple pairing one member's element with the
// other member's.
...
// fusableTuples reports whether two tuple atoms line up position for position. A
// `...P` spread element rules the pair out: the element list is not yet the
// tuple's real positions, so pairing them up would compare a spread against an
// ordinary element.
func fusableTuples(a, b *soltype.TupleType) bool {
...
// where the fusion is exact.
//
//   - The domains agree: `(A -> C) & (A -> D)` is `A -> (C & D)`. A value of both
//     types, applied to an A, returns something both codomains admit.
//   - The codomains agree and there is one parameter: `(A -> C) & (B -> C)` is
//     `(A | B) -> C`. A value of both types accepts an A or a B and returns a C
//     either way.
//
// Everything else keeps both atoms, which is the shape decision this file's header
// records. MLscript fuses EVERY pair to `(l0 | l1) -> (r0 & r1)` and then applies
// the plain arrow rule to the result, which accepts
// `(number -> boolean) & (string -> null) <: (number | string) -> boolean` by
// meeting the codomains to `never`. That is unsound: the value's string arm really
// does return a null where the target promises a boolean. Keeping the arms apart
// is what lets PR5 (#1062) decide such a constraint by the
// Frisch-Castagna-Benzaken decomposition instead.
//
// The codomain-agreeing case is restricted to a single parameter because unioning
// each parameter position independently is wrong for a longer signature.
// `(number, number) -> C` and `(string, string) -> C` do not fuse to
// `(number | string, number | string) -> C`, which would claim the value accepts a
// number paired with a string.
...
// to build one arrow from them. Everything the fused arrow does not recompute must
// already agree: the arity, the surface markers on each parameter, the trailing
// `...`, and what a call may raise. A receiver or a quantifier rules the pair out,
// since a fused arrow would have to say which receiver it takes or how the two
// binder lists correspond.
//
// Requiring the raised types to be equal is conservative. `(A -> C throws E) &
// (A -> C throws F)` really raises `E & F`, since a value of both types can only
// raise what both permit, but merging the raise position belongs with PR9's throws
// threading. TODO(#1066).
...
// object carries any other member kind. A method, an accessor, a constructor, a
// spread, and a mapped member each need their own merge rule, so an object
// carrying one is kept as an unfused atom rather than merged under a property-only
...
func (c *Context) canonicalConjuncts(conjuncts []Conjunct) []Conjunct {
	...
	merged := mergeAll(fused, c.tryMergeUnion, sortConjuncts)
...
func (c *Context) normalizeConjunct(a Conjunct) (Conjunct, bool) {
	lnf, inhabited := c.fuseAtoms(copyAtoms(a.Lnf.Atoms), meetOfAtoms)
	if !inhabited {
		return Conjunct{}, false
	}
	rnf, _ := c.fuseAtoms(copyAtoms(a.Rnf.Atoms), joinOfAtoms)
...
func (c *Context) canonicalDisjuncts(disjuncts []Disjunct) []Disjunct {
	...
	merged := mergeAll(fused, c.tryMergeInter, sortDisjuncts)
...
// result in canonical order. reorder restores that order before the first scan and
// after each fusion, so which pair fuses is decided by the order the members sort
// in rather than by the order they arrived in — the same reason fuseAtoms sorts
// between fusions. Each fusion shortens the list, so the loop runs at most
// len(items) times.
//
// items is reordered in place, so a caller that still needs the input order must
// pass a copy.
func mergeAll[T any](items []T, try func(a, b T) (T, bool), reorder func([]T)) []T {
	reorder(items)
	for {
		fused, changed := mergePass(items, try)
		if !changed {
			return fused
		}
		reorder(fused)
...
func mergePass[T any](items []T, try func(a, b T) (T, bool)) ([]T, bool) {
