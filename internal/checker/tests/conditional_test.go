...
func TestConditionalTypeAliasBasic(t *testing.T) {
	tests := map[string]struct {
		...
		// TODO: tuple spread types are not yet supported
		...
		"DistributiveConditionalTypesProducesCartesianProduct": {
			input: `
				type Combine<T, U> = if T : any { if U : any { {first: T, second: U} } else { never } } else { never }
				type MultiUnion = Combine<string | number, boolean | null>
			`,
			expectedTypes: map[string]string{
				"MultiUnion": "{first: string, second: boolean} | {first: string, second: null} | {first: number, second: boolean} | {first: number, second: null}",
			},
		},
...
func TestConditionalTypeAliasAdvanced(t *testing.T) {
	tests := map[string]struct {
		...
		// TODO: Update how rest params in function types are unified
		// We should be using tuple types for rest params when possible
		...
		"ChainedConditionalTypes": {
			...
			`,
			expectedTypes: map[string]string{
				"Result1": "IsStringOrNumber<string>",
				"Result2": "IsStringOrNumber<boolean>",
				"Result3": "false",
			},
		},
...
func TestConditionalTypeAliasEdgeCases(t *testing.T) {
	tests := map[string]struct {
		...
		// 	`,
		// 	expectedTypes: map[string]string{
		// 		"Result1": "string", // DeepFlatten<Array<any>>
		// 		"Result2": "string", // Array<string>
		// 		"Result3": "string",
		// 	},
		// },
		// TODO: enable once mapped types are implemented
