...
func TestMutabilityTransitions(t *testing.T) {
	tests := map[string]struct {
		...
		"PropertyAssignment_MergesAliasSets_Error": {
			input: `
				fn test() {
		...
		"PropertyAssignment_FreshValue_OK": {
			input: `
				fn test() {
					val obj: mut {prop: mut {x: number}} = {prop: {x: 0}}
					obj.prop = {x: 1}
		...
		"Conditional_IfElse_AliasesBothBranches_Error": {
			input: `
		...
		"Conditional_IfElse_BothBranchesViolate_MultipleErrors": {
			input: `
				fn test(cond: boolean) {
					val a: mut {x: number} = {x: 0}
					val b: mut {x: number} = {x: 1}
		...
		"ClosureCapture_ReadOnly_MutSourceWithLiveMutAlias_Error": {
			input: `
				fn test() {
		...
		"ClosureCapture_ReadOnly_BlocksImmutableToMut": {
			input: `
				fn test() {
		...
		"ClosureCapture_MutableCapture_BlocksMutToImmutable": {
			input: `
				fn test() {
					val items: mut {x: number} = {x: 0}
					val f = fn() { items.x = 1 }
		...
		"ClosureCapture_ReadOnly_DoesNotBlockMutToImmutable": {
			input: `
				fn test() {
					val items: mut {x: number} = {x: 0}
					val f = fn() -> {x: number} { items }
		...
		"ClosureCapture_PrimitiveReassign_NoTransitionError": {
			input: `
				fn test() {
					var count = 0
					val count2 = count
		...
		"ClosureCapture_LiteralTypeReassign_NoTransitionError": {
			input: `
				fn test() {
					var label: "on" | "off" = "on"
					val label2 = label
		...
		"ClosureCapture_ShadowedByForIn_CapturesOuterVar_Error": {
			input: `
				fn test() {
					val items: mut {x: number} = {x: 0}
					for item in [1, 2, 3] {
		...
		"Conditional_Reassignment_DoesNotMergeUnrelatedSets_OK": {
			input: `
				fn test(cond: boolean) {
					val a: mut {x: number} = {x: 0}
					val b: mut {x: number} = {x: 1}
		...
		"Reassignment_LeavesAliasSet_OK": {
			input: `
				fn test() {
					val a: mut {x: number} = {x: 0}
		...
		"Reassignment_ImmutableToMut_Error": {
			input: `
				fn test() {
					val a: mut {x: number} = {x: 0}
					var b: {x: number} = {x: 1}
		...
		"Conditional_Reassignment_Error": {
			input: `
				fn test(cond: boolean) {
					val a: mut {x: number} = {x: 0}
					val b: mut {x: number} = {x: 1}
		...
		"Reassignment_FreshAfterAlias_ClearsConflict_OK": {
			input: `
				fn test() {
					val a: mut {x: number} = {x: 0}
					var b: {x: number} = a
		...
		"Match_EnumDestructuring_ConditionalAlias_Error": {
			input: `
				enum Maybe {
					Left(value: number),
					Right(value: number),
		...
		"Match_EnumDestructuring_ConditionalAlias_OK": {
			input: `
				enum Maybe {
					Left(value: number),
					Right(value: number),
		...
		"Conditional_SourceInMultipleSets_NoDuplicateConflicting": {
			input: `
				fn test(cond: boolean) {
					val a: mut {x: number} = {x: 0}
					val b: mut {x: number} = {x: 1}
