...
func TestAsyncFunctionInferenceScript(t *testing.T) {
	tests := map[string]struct {
		...
		"AsyncFuncWithInferredReturnType": {
			...
			`,
			expectedFn:   "fn (url: string) -> Promise<mut? \"data\", never> throws never",
		...
		"AsyncFuncWithThrow": {
			...
			`,
			expectedFn:   "fn (url: string) -> Promise<mut? \"data\", mut? \"error\"> throws never",
			functionName: "fetchData",
			expectedErr:  false,
		},
		"SimpleDeclareTest": {
		...
		"AsyncFuncWithAwait": {
			...
			`,
			expectedFn:   "fn (url: string) -> Promise<Response, never> throws never",
			functionName: "fetchData",
			expectedErr:  false,
		},
		...
		"AsyncFuncWithMultipleThrowTypes": {
			...
			`,
			expectedFn:   "fn (flag: boolean) -> Promise<mut? \"data\", mut? \"string error\" | mut? 42> throws never",
			functionName: "fetchData",
			expectedErr:  false,
		},
		"AsyncFuncWithAwaitAndThrow": {
			...
			`,
			expectedFn:   "fn (url: string) -> Promise<Response, mut? \"invalid url\"> throws never",
			functionName: "fetchData",
			expectedErr:  false,
		},
		"AsyncFuncWithNestedAsync": {
			...
			`,
			expectedFn:   "fn (url: string) -> Promise<Response, mut? \"inner error\"> throws never",
...
func TestAsyncFunctionInferenceModule(t *testing.T) {
	tests := map[string]struct {
		input        string
		expectedFn   string // expected function type
		functionName string // function name to check (defaults to "fetchData")
	...
	for testName, testCase := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			if testCase.expectedErr {
				...
				// Check the function type by looking it up in the result scope
				if testCase.expectedFn != "" && testCase.functionName != "" {
					binding := scope.Values[testCase.functionName]
					assert.NotNil(t, binding, "Function binding should be found in scope")
					if binding != nil {
						actualType := binding.Type.String()
						t.Logf("Actual function type: %s", actualType)
